...
class Streak(object):
    def __init__(self, tem_score=0, last_date=today, 
                 tem_days=0, best_days=0, total=0, *args, **kwargs):
        # 临时记录
        self.tem_score = tem_score
        # 上次记录时间
        try:
            self.last_date = last_date.date()
        except:
    ...
    def __repr__(self):
        return '<Streak: %s>' % str((self.streak, self.best))
...
class StreakField(models.Field):
    ...
    def to_python(self, value):
        ...
        return Streak(tem_score, last_date, tem_days, best_days, total)

    def get_db_prep_value(self, value, *args, **kwargs):
        return str(
            (value.tem_score, value.last_date.strftime("%Y-%m-%d"),
             value.tem_days, value.best_days, value.total))
